...
def test_get_account_balance():
    ...
    transaction1 = Transaction(
        mock_account_id, Transaction.TransactionType.CREDIT, 400, datetime.now().date()
    )
    ...
    transaction3 = Transaction(
        mock_account_id, Transaction.TransactionType.DEBIT, 23.47, datetime.now().date()
    )
    ...
    transaction5 = Transaction(
        mock_account_id, Transaction.TransactionType.CREDIT, 600, datetime.now().date()
    )
    assert ledger.is_empty
    transactions = [
        transaction1,
